...
def loadModel():
    """
    Load the model with weights from the current directory.
    ...
    # Use the current directory to load the weights
    print("Current working directory:", os.getcwd())

    weights_path = "/opt/ml/model/arcface_weights.h5"

    if not os.path.isfile(weights_path):
        raise ValueError("arcface_weights.h5 weights file is not found in the current directory.")

    model.load_weights(weights_path)

    return model

    # Note: This modified function expects the weights.h5 file to be present in the "asd" directory by default.
    # You can also specify a different path when calling the function, e.g., modified_loadModel("path/to/weights.h5").
def ResNet34():
