...
def load_weights(model):
    # 현재 디렉토리의 retina_weights.h5 파일 경로를 설정
    exact_file = 'retina_weights.h5'

    # 해당 파일이 현재 디렉토리에 존재하는지 확인
    if not os.path.isfile(exact_file):
        raise ValueError(f"File {exact_file} does not exist in the current directory. Please ensure it's present.")
